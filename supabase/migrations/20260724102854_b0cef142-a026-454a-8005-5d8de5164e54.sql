...
SET approval_status = 'Approved',
    approved_at = COALESCE(pb.approved_at, pb.imported_at, pb.created_at)
WHERE pb.bill_type = 'Pharmacy'
  AND EXISTS (
    SELECT 1 FROM public.purchase_bill_items pbi
...
BEGIN
  ...
  INSERT INTO public.purchase_bills (
    ...
    payment_mode, payment_status, approval_status, approved_at, notes,
    ...
    COALESCE(NULLIF(_invoice->>'subtotal','')::numeric, NULLIF(_invoice->>'totalAmount','')::numeric, 0),
    COALESCE(NULLIF(_invoice->>'gstAmount','')::numeric, 0),
    COALESCE(NULLIF(_invoice->>'discount','')::numeric, 0),
    COALESCE(NULLIF(_invoice->>'totalAmount','')::numeric, 0),
    'Pending', 'Pending', 'Approved', now(),
    COALESCE(NULLIF(_audit->>'notes',''), 'Imported via AI scanner'),
    NULLIF(_supplier->>'gst',''),
    NULLIF(_supplier->>'address',''),
    NULLIF(_supplier->>'contact',''),
    COALESCE(NULLIF(_invoice->>'roundOff','')::numeric, 0),
    COALESCE(NULLIF(_invoice->>'netPayable','')::numeric, NULLIF(_invoice->>'totalAmount','')::numeric, 0),
    ...
    COALESCE(NULLIF(_audit->>'approved_by','')::uuid, _user_id),
  ...
  LOOP
    ...
    _qty := COALESCE(NULLIF(_item->>'quantity','')::integer, 0);
    _free := COALESCE(NULLIF(_item->>'freeQuantity','')::integer, 0);
    ...
    IF _med_id IS NULL THEN
      INSERT INTO public.medicines (
        ...
        COALESCE(NULLIF(_item->>'mrp','')::numeric, 0),
        COALESCE(NULLIF(_item->>'sellingRate','')::numeric, NULLIF(_item->>'mrp','')::numeric, 0),
        NULLIF(_item->>'hsnCode',''),
        COALESCE(NULLIF(_item->>'gstPercent','')::numeric, 12),
    ...
    INSERT INTO public.purchase_bill_items (
      ...
      NULLIF(_item->>'expiryDate','')::date,
      _qty, _free,
      COALESCE(NULLIF(_item->>'purchaseRate','')::numeric, 0),
      COALESCE(NULLIF(_item->>'mrp','')::numeric, 0),
      NULLIF(_item->>'sellingRate','')::numeric,
      COALESCE(NULLIF(_item->>'gstPercent','')::numeric, 12),
      NULLIF(_item->>'hsnCode',''),
      COALESCE(NULLIF(_item->>'amount','')::numeric, COALESCE(NULLIF(_item->>'purchaseRate','')::numeric,0) * _qty),
